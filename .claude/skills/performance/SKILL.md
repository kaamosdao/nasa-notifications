...
Where each lever below already lives in this stack (don't reinvent them):
...
- **Server response / caching** — global CMS data is cached with **lru-cache** (`cached()`,
  ...
  cache headers are set at **nginx**, not in app code.
- **JS splitting** — use **`next/dynamic`** for heavy client-only components; keep server work in
  `getServerSideProps`.
- **CSS / fonts** — SCSS modules are auto-injected and Next handles critical CSS
  ([writing-styles](../writing-styles/SKILL.md)); no custom web-fonts are wired yet (system stack).
- **Scroll / third-party / runtime** — smooth scroll is **Lenis** and animation is GSAP with strict
  unmount cleanup; consume scroll via `useScroll`, never raw listeners. See
  [animation](../animation/SKILL.md).
- **Tooling** — **pnpm** + **biome**; no service worker and no `web-vitals` lib are wired (add only
  if the project needs them). The generic service-worker/HTTP-header snippets below are reference,
  not current setup.
