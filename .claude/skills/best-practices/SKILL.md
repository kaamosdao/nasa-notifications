...
The generic patterns below hold; here's where they actually live in this stack.

- **Package manager is pnpm, linter is biome.** Use `pnpm audit` (not `npm`/`yarn`); there is no
  eslint/webpack config to edit — the build is Next.js (`next.config.js`) and the prod image runs
...
- **Memory leaks / event listeners / passive scroll**: don't attach raw `wheel`/`scroll`/`resize`
  listeners — scroll is **Lenis**-driven and consumed via `useScroll`; all GSAP/observer/listener
  teardown on unmount is covered in [animation](../animation/SKILL.md) (cleanup discipline).
