...
Pages Router + SSR. Apply the metrics below through the real stack — **ignore the generic
`next/image` advice, this project doesn't use it** (see the corrected framework section at the end).
...
- **CLS** — `MediaImage` already carries `width`/`height` from `MediaWithBreakpoints`, so images
  ...
  (system stack), so FOUT/`font-display` is a per-project concern once fonts are added.
- **INP** — client state is **zustand** ([code-conventions](../code-conventions/SKILL.md)); split
  heavy client-only components with `next/dynamic`. Scroll work runs through Lenis — don't add raw
  scroll handlers.
- **Measurement** — `web-vitals` is **not** a dependency; add it only if you need field data.
...
### Next.js (this project — Pages Router, NO next/image)
...
// INP: split heavy client-only components
import dynamic from "next/dynamic";
const HeavyComponent = dynamic(() => import("./heavy"), { ssr: false });

// CLS: MediaImage carries width/height from MediaWithBreakpoints — space is reserved.
//      Data comes from getServerSideProps, so the LCP content is in the initial HTML.
```
