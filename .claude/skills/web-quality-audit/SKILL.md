...
- **`scripts/analyze.sh` scans static `.html`/`.htm` files.** This app renders **`.tsx`** — running
  it against `src/` finds nothing. Point it at **served HTML**: `curl -s localhost:3000/ > page.html`
  (dev server running) or the `.next` server output, then `scripts/analyze.sh page.html`.
- **Tooling**: **pnpm** (not npm), **biome** (not eslint). No Lighthouse/axe CI is wired — run audits
  against the **running dev server**.
- **SEO / OG / robots / canonical** are handled by the `SeoLayout` widget and the `seo` skill — audit
  the rendered `<head>`, don't add per-page tags.
- **Security headers / CSP** live at **nginx** (Ansible), not in app code; images/HTTPS via imgproxy.
