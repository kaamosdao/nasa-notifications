...
Generic WCAG advice below is correct; apply it through **this stack**, not raw HTML. This is a
Next.js **Pages Router** app with SCSS modules — a11y lives in `.tsx` components + `.module.scss`,
not in static pages.

- **Visually-hidden text**: use the existing **`sr-only()`** mixin (auto-injected in every
  `.module.scss`, see [writing-styles](../writing-styles/SKILL.md)) — don't hand-roll a
  `.visually-hidden` class.
- **Focus / target-size / contrast**: use CSS tokens (`--c-*`) and the `mq()` mixin, not hardcoded
  px/colors. Focus rings and 24×24 targets go in the component's module.
- **Reduced motion**: the site's motion is GSAP + Lenis. `prefers-reduced-motion` must gate those
  timelines too, not just CSS transitions — see [animation](../animation/SKILL.md).
- **Forms** (labels, `aria-invalid`, error announcement): forms are react-hook-form + Zod through
  `shared/ui` primitives — wire labels/aria in those primitives. See [forms](../forms/SKILL.md)
  (note: `Input` needs `Controller`, not bare `register`).
- **Semantics / interactive elements**: prefer native `<button>`/`<a>`; component structure per
  [creating-a-component](../creating-a-component/SKILL.md).
- **Page `lang`**: set once in `_document`/`_app` (single locale today; multilingual `hreflang`
  is a per-project TODO — see [seo](../seo/SKILL.md)).
- **Tooling**: package manager is **pnpm** (`pnpm dlx @axe-core/cli …`); linter is **biome**.
  No Lighthouse CI is wired — run audits against the **running dev server**, not `src/`.
