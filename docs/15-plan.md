...
Статус: **этапы 0–4 выполнены, этап 5 — без деплоя и переписывания документации.** Этап 0 — CMS-слой, imgproxy и Meilisearch удалены из кода,
...
SSR первых 20, SSE-клиент, анимации вставки, история вверх. Этап 4 — hero: `widgets/hero-metaballs`,
WebGL2-рендерер с фолбэками. Этап 5 — переход intro → background, композиция слоёв, a11y и SEO/OG;
остались деплой (compose + CI + nginx под SSE) и переписывание boilerplate-документации.
...
- **Небо:** то же `env(rd, spread)` для лучей мимо шара, один источник для фона и для отражения —
  они согласованы по определению. Небо физичное, а не декоративное: звезда — точка на небесной
  сфере с явным угловым размером (не диск в ячейке хеш-сетки), светимости распределены степенным
  законом, цвет считается по температуре чёрного тела, у ярких есть гало и дифракционный крест.
  Поверх — процедурный диск Галактики с прожилками пыли; к его плоскости звёзды сгущаются.
  `spread` — угловой радиус самой мелкой детали: для фона это пиксель (меньше нельзя — звёзды
  начинают мерцать на движении камеры), для отражения он в разы больше, потому что кривизна шара
  сжимает в один пиксель целый кусок неба. Финальный тонмап — Рейнхард, иначе ядра ярких звёзд
  клиппятся в плоские белые пятна.
...
- `widgets/hero-metaballs/`: `lib/renderer.ts` (класс `HeroRenderer` — контекст, rAF, слушатели),
  `lib/shaders/*.ts`, `model/hero-store.ts` (фаза intro/background), компонент-обёртка.
- **Шейдеры — TS-строки, а не `.glsl`-файлы.** Отдельный webpack-loader ради двух файлов не окупается,
  а строка даёт подсветку через комментарий `/* glsl */` без правки `next.config.js`.
- Вся мутабельная механика в классе, React только создаёт/уничтожает рендерер и переключает фазу.
  Пружина курсора, лерп uniform'ов и разрешение считаются от `delta`, а не от номера кадра.
- Канвас смонтирован в `_app` (выше страницы), фиксированным слоем с `z-index: 0`; страница ленты
  получила `position: relative; z-index: 1` — без явного слоя fixed-канвас перекрывает контент.
- `--main-background-color` переехал с графита на `--c-space` (#05060a): под канвасом лежит
  постер-фолбэк, и графитовый фон просвечивал бы серым.
- Разрешение = `min(DPR, 1.75) × качество × масштаб фазы`; в фазе background — 0.6, при росте
  `performanceIndex` из `widgets/performance-detect` падает до 0.45. Шаги raymarching фиксированы:
  дешевле резать пиксели, чем ветвиться в шейдере.
- `document.hidden` останавливает rAF, `delta` клампится 1/30 — иначе после возврата из фона
  пружина курсора получает дельту в минуты и взрывается.
- CSS-размеры канваса держим от `ResizeObserver`: `getBoundingClientRect()` каждый кадр — чтение
  layout в rAF.
- `prefers-reduced-motion` → один статичный кадр (перерисовывается на resize), указатель
  не отслеживается. Нет WebGL2 → постер под канвасом (он лежит там всегда, канвас проявляется поверх).
- Проверено в headless Chrome (ANGLE Metal, M4): 60 FPS в fullscreen-фазе; смена фазы режет буфер
  1440×813 → 864×488; после unmount `gl.isContextLost() === true` (в `destroy()` дёргаем
  `WEBGL_lose_context`, иначе контекст живёт до GC); при `prefers-reduced-motion` два скриншота
  с интервалом 2.5 с побайтово совпадают; с заглушённым `getContext('webgl2')` виден постер.
- ⚠️ Переключателя фазы в интерфейсе пока нет — таймлайн intro → background и hero-текст на этапе 5.
...
- `widgets/hero-intro` — текстовый слой intro (заголовок посимвольно через `SplitText`,
  подзаголовок, кнопка «Открыть поток»), смонтирован в `_app` рядом с канвасом.
- **Общего таймлайна на все слои нет — у каждого свой, согласование по фазе в сторе.**
  План предполагал один таймлайн с `.reverse()`, но слои монтируются независимо (текст — в
  layout, карточки — в странице и меняются потоком SSE), поэтому собранный один раз таймлайн
  указывал бы на устаревший DOM. Прерывание на полпути гасится `overwrite: "auto"`, а не
  проигрыванием того же таймлайна назад.
- Обратный переход — кнопка «↑ К началу» в хедере и `Esc` (`widgets/hero-intro/lib/use-hero-hotkeys`,
  фаза читается через `getState()`, чтобы слушатель не переподключался).
- **Скрытые слои закрываются `inert`, а не только прозрачностью**: `autoAlpha` убирает их из
  `visibility`, `inert` — из таб-порядка и из дерева доступности.
  Стартовое состояние ленты (`visibility: hidden`) задано классом: до первого кадра JS
  её не должно быть видно.
- Страница ленты рисует `div`, а не `main`: `main` уже отдаёт `TransitionLayout`, два landmark'а
  ломают навигацию скринридера.
- В фоновой фазе камера дополнительно смещается вбок (`uOffset`, доля полуширины кадра, не
  мировые единицы) — иначе шар остаётся ровно под лентой и его не видно.
- `prefers-reduced-motion` теперь гасит и CSS-переходы (правило в `shared/styles/_base.scss`):
  JS-анимации мы отключаем сами, но затухания слоёв живут в стилях.
- DOM-курсор из boilerplate убран из `_app`: он рисовал название своего типа («default») поверх
  hero, а роль курсора здесь играет шар в шейдере.
- SEO: `APP_INFO` заполнен (title/description/keywords, `theme-color` = `--c-space`),
  `/og.png` — кадр живой сцены 1200×630, `<html lang="ru">`.
- Проверено в headless Chrome: intro → background → `Esc` → кнопка хедера, состояния слоёв
  (`opacity`/`visibility`/`inert`) на каждом шаге, 60 FPS после добавления `uOffset`,
  при `prefers-reduced-motion` переход мгновенный.

Осталось в этапе: деплой (compose + CI + nginx под SSE) и переписывание документов с
boilerplate-баннером (`docs/03`, `04`, `08`, `10`, `13`).
